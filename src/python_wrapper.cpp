...
void* train(py::array_t<double> Y, py::object general_X, std::string model_name, int iter, int leaves, double shrinkage, py::kwargs params = py::none()) {
  ...
  ONLINEGBDT::Config* config = new ONLINEGBDT::Config();
  ...
  ONLINEGBDT::Data* data = new ONLINEGBDT::Data(config);

  ONLINEGBDT::GradientBoosting* model;
...
py::dict test(py::array_t<double> Y, py::object general_X, void* py_model, py::kwargs params = py::none()) {
  ...
  ONLINEGBDT::GradientBoosting* model =
      reinterpret_cast<ONLINEGBDT::GradientBoosting*>(py_model);
  ONLINEGBDT::Config* config = model->getConfig();
  if (params != py::none()){
    for(auto it : params){
      const std::string& name = py::cast<const std::string>(it.first);
      std::string val = py::cast<const std::string>(py::str(it.second));
...
void* loadModel(std::string path) {
  ...
  ONLINEGBDT::ModelHeader model_header = ONLINEGBDT::GradientBoosting::loadModelHeader(config);
...
PYBIND11_MODULE(onlineGBDT, m) {
    m.doc() = "ONLINEGBDT is a high-performance gradient boosting library for adaptive base class training."; // optional module docstring
    m.def("train", &train, "train ONLINEGBDT model",
          py::arg("Y"),
          py::arg("X"),
          py::arg("model_name"),
          py::arg("iter"),
    ...
    m.def("test",&test, "test ONLINEGBDT model",
          py::arg("Y"),
          py::arg("X"),
          py::arg("model"));
    m.def("predict",&predict, "predict ONLINEGBDT model",
          py::arg("X"),
          py::arg("model"));
    m.def("load",&loadModel,"load ONLINEGBDT model",
          py::arg("path"));
    m.def("save",&saveModel,"save ONLINEGBDT model",
          py::arg("model"),
          py::arg("path"));
}
