...
onlineGBDT_train <- function(train_Y,train_X,model_name,iter,leaves,shinkage,params=NULL){
...
onlineGBDT_test <- function(test_Y,test_X,model,params=NULL){
...
onlineGBDT_predict <- function(test_X,model,params=NULL){
  if(is(test_X,'sparseMatrix') == FALSE){
    test_X = as.matrix(test_X)
    if(is.matrix(test_X) == FALSE){
      stop("test_X must be a 2 dimensional matrix.")
...
onlineGBDT_save_model <- function(model,path){
	.Call("saveModel",model,path)
}

onlineGBDT_load_model <- function(path){
	model <-.Call("loadModel",path)
	return(model)
}


onlineGBDT_read_libsvm <- function(path){
